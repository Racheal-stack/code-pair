        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-secondary/50 backdrop-blur-sm border border-border rounded-lg p-8">
            <h1 className="text-2xl font-bold mb-2">
              {invitationInfo ? 'Sign In to Join Interview' : 'Welcome Back'}
            </h1>
            <p className="text-foreground/60 mb-6">
              {invitationInfo 
                ? 'Sign in to access the interview environment'
                : 'Sign in to your CodePair account'
              }
            </p>

            <LoginForm invitationInfo={invitationInfo} />

            <p className="mt-6 text-center text-foreground/60">
              Don't have an account?{" "}
              <Link 
                href={invitationInfo ? `/signup?invitation=${encodeURIComponent(JSON.stringify(invitationInfo))}` : "/signup"}
